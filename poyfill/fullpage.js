...
(function(window) {
  ...
  var passiveListener = passiveSupported
    ...
    initEvent() {
      ...
      if (that.o.drag) {
        ele.addEventListener('touchmove', function (e) {
          ...
          if ((that.curIndex == 0 && y > 0) ||
            (that.curIndex === that.pagesLength - 1 && y < 0))
            y /= 2;
          var x = e.changedTouches[0].pageX - that.startX;
          if ((that.curIndex == 0 && x > 0) ||
            (that.curIndex === that.pagesLength - 1 && x < 0))
            x /= 2;
          var dist = that.o.dir === 'v'
            ? -that.curIndex * that.height + y
            : -that.curIndex * that.width + x;
